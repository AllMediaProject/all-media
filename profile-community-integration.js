...
const bars=[...document.querySelectorAll("[data-profile-community-bar]")];
...
function closeAllMoreMenus(){
  bars.forEach(bar=>{
    bar.querySelector("[data-profile-community-dropdown]")
      ?.classList.remove("active");

    bar.querySelector("[data-profile-community-more]")
      ?.setAttribute("aria-expanded","false");
  });
}

function renderBar(bar){
  const hotbar=bar.querySelector("[data-profile-pinned-communities]");
  const dropdown=bar.querySelector("[data-profile-community-dropdown]");
  ...
  pinnedOrder.forEach(slug=>{
    ...
    chip.innerHTML=`
      ...
      <span class="profile-community-chip-name"></span>
      ${bellMarkup(slug)}
    `;

    chip.querySelector(".community-icon").textContent=item.icon||"◉";
    chip.querySelector(".profile-community-chip-name").textContent=item.name||slug;

    chip.addEventListener("click",event=>{
      const bell=event.target.closest("[data-profile-community-bell]");
  ...
  joined.forEach(([slug,item])=>{
    ...
    name.innerHTML=
      '<span class="community-icon"></span><span class="profile-community-more-text"></span>';

    name.querySelector(".community-icon").textContent=item.icon||"◉";
    name.querySelector(".profile-community-more-text").textContent=item.name||slug;
...
function wireChipDrag(chip,hotbar){
  chip.addEventListener("dragstart",event=>{
    if(event.target.closest("[data-profile-community-bell]")){
      event.preventDefault();
      return;
    }
...
bars.forEach(bar=>{
  const hotbar=bar.querySelector("[data-profile-pinned-communities]");
  const moreButton=bar.querySelector("[data-profile-community-more]");
  const manageButton=bar.querySelector("[data-profile-manage-communities]");
...
function openQuickNotifications(slug,anchor){
  const popup=document.getElementById("profileCommunityQuickPopup");
  const list=document.getElementById("profileCommunityQuickList");
  ...
  document.getElementById("profileCommunityQuickTitle").textContent=
  ...
  if(!items.length){
    ...
    empty.className="profile-community-quick-empty";
    empty.innerHTML=
    ...
    items.forEach(item=>{
      ...
      row.type="button";
      row.className=
        "profile-community-quick-item"+(read.has(item.id)?"":" unread");

      row.innerHTML=
        '<span class="profile-community-quick-item-copy"><strong></strong><span></span></span><small></small>';

      row.querySelector("strong").textContent=item.title;
      row.querySelector(".profile-community-quick-item-copy > span")
  ...
  document.getElementById("profileCommunityQuickFooter").href=
...
document.getElementById("profileCommunityQuickClose")
  ?.addEventListener("click",closeQuickNotifications);

function openManageDrawer(){
  const layer=document.getElementById("profileManageCommunitiesDrawerLayer");
  const frame=document.getElementById("profileManageCommunitiesDrawerFrame");
...
document.getElementById("profileManageCommunitiesDrawerClose")
  ?.addEventListener("click",closeManageDrawer);

document.getElementById("profileManageCommunitiesDrawerBackdrop")
  ?.addEventListener("click",closeManageDrawer);

document.addEventListener("click",event=>{
  if(!event.target.closest("[data-profile-community-bar] .community-menu")){
    closeAllMoreMenus();
  }
  ...
  if(
    popup &&
    !popup.hidden &&
    !event.target.closest("#profileCommunityQuickPopup") &&
    !event.target.closest("[data-profile-community-bell]")
  ){
    popup.hidden=true;
  }
});
