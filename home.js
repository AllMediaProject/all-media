...
function toggleManageInterests(){const p=document.getElementById("interestManagePanel"),b=document.getElementById("manageInterestsButton");p.classList.toggle("open");const open=p.classList.contains("open");b.classList.toggle("active",open);const label=b.querySelector(".manage-collections-label");if(label)label.textContent=open?"Close Collections":"Manage Collections";renderHashtagCollections()}
...
function updateMoreLink(post){
    if(post?.classList.contains("regular-post")||post?.classList.contains("blog-post-v11")||post?.classList.contains("video-post-v11")||post?.classList.contains("byte-post-v11")) return;
...
function togglePostHashtags(el){
  ...
  /* Keep the drawer in the exact approved lab position:
     immediately after the quick-comment/footer row. */
  ...
  /* Force the approved lab geometry inline so none of the older Home
     hashtag rules can resize or reposition this drawer. */
...
function splitBlogFeedContent(html,fallbackText){
    const shell=document.createElement("div");
    shell.innerHTML=html||"";
    let target=null;

    for(const node of [...shell.childNodes]){
        if(node.nodeType===1){
            const el=node;
            if(el.matches?.(".blog-inline-media,figure") && !((el.textContent||"").trim())) continue;
        }
        const raw=(node.textContent||"").replace(/\u00a0/g," ").trim();
        if(raw){target=node;break;}
    }

    if(!target){
        const clean=(fallbackText||"").trim();
        return{preview:clean.length>420?clean.slice(0,420).trim()+"…":clean,continuation:""};
    }
    ...
    if(raw.length>420){
        cut=420;
        const wordBreak=raw.lastIndexOf(" ",420);
        if(wordBreak>=320)cut=wordBreak;
    }

    const preview=raw.slice(0,cut).trim()+(cut<raw.length?"…":"");

    if(cut>=raw.length){
        target.remove();
    }else{
        const remainder=raw.slice(cut).trimStart();
        if(target.nodeType===3){
            target.textContent=remainder;
        }else{
            target.textContent=remainder;
        }
    }

    return{preview,continuation:shell.innerHTML};
}
...
function createPost(){
    ...
    const videoHTML=videoItem?`<div class="video-feed-player ${mode==="video"?"video-v11-feed-player":mode==="byte"?"byte-feed-player byte-v11-feed-player":""}">${videoPoster?`<div class="video-feed-poster"><img src="${videoPoster.url}" alt="Video preview image" style="${composerImageStyle(videoPoster)}"></div>`:""}<video preload="metadata" playsinline src="${videoItem.feedUrl}" data-video-type="${escapeHTML(videoItem.type||'')}" onclick="showVideoControls(this.closest('.video-feed-player'))"></video><div class="video-playback-warning">This browser cannot play this video file.</div>${videoControlHTML()}</div>`:"";
    const contentHTML=mode==="blog"?`<div class="blog-feed-title">${escapeHTML(title)}</div>${imageHTML?`<div class="blog-home-preview">${imageHTML}</div>`:""}<div class="caption blog-feed-excerpt">${escapeHTML(blogFeedParts.preview)}</div><button type="button" class="blog-read-button" onclick="toggleBlogBody(this)">Read Blog</button><div class="blog-full-body">${blogFeedParts.continuation}</div><template class="blog-source-html">${blogHTML}</template>`:mode==="video"?`<div class="video-feed-title">${escapeHTML(title)}</div>${videoHTML}<div class="video-caption-row"><div class="caption video-caption">${escapeHTML(text)}</div><button type="button" class="video-caption-more" onclick="toggleVideoDescription(this)">More</button></div>`:mode==="byte"?`<div class="byte-stage">${videoHTML}</div><p class="byte-caption caption">${escapeHTML(text)}</p>`:`${imageHTML}<div class="caption">${escapeHTML(text)}</div>`;
    if(mode==="post"){
...
<p class="blog-excerpt blog-feed-excerpt">${escapeHTML(blogFeedParts.preview)}</p>
<button class="read-blog blog-read-button" onclick="blogV11ToggleBlog(this)" type="button">Read Blog →</button>
<div class="blog-full-content blog-full-body">${blogFeedParts.continuation}</div>
<template class="blog-source-html">${blogHTML}</template>
<div class="interactions">
...
<div class="post-header-right">
<button aria-label="More options" class="post-menu-button" onclick="videoV11TogglePostMenu(event,this)" type="button">•••</button>
<div class="video-v11-menu">
<button class="post-menu-action" onclick="editPost(this)" type="button">Edit</button>
<span class="post-menu-divider">|</span>
<button class="post-menu-action post-menu-delete" onclick="deletePost(this)" type="button">Delete</button>
</div>
...
<h2 class="video-title"><span aria-hidden="true" class="video-title-icon">▶</span>${escapeHTML(title)}</h2>
${videoHTML}
<p class="video-description caption video-caption">${escapeHTML(text)}</p>
<div class="interactions">
<button class="action-pill video-like-button" onclick="videoV11ToggleLike(this)" type="button"><svg aria-hidden="true" class="like-heart" viewBox="0 0 24 24"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8Z"></path></svg><span class="like-count">0</span></button>
<button class="action-pill pocket-action" type="button"><svg aria-hidden="true" class="action-icon" viewBox="0 0 24 24"><path d="M4 7h6l2 2h8v10H4Z"></path><path d="M8 4v6"></path></svg><span>Pocket</span></button>
<button class="action-pill community-action" type="button"><svg aria-hidden="true" class="action-icon" viewBox="0 0 24 24"><circle cx="8" cy="8" r="3"></circle><circle cx="17" cy="9" r="2.5"></circle><path d="M3 19c.4-4 2.4-6 5-6s4.6 2 5 6"></path><path d="M13 18c.5-2.8 1.9-4.3 4-4.3 2 0 3.4 1.4 4 4.3"></path></svg><span>Community</span></button>
<button class="reblog-counter" onclick="videoV11IncrementReblog(this)" type="button">↻ <span>0</span> · Reblog</button>
<button class="share-action" type="button">↗ Share</button>
<button aria-label="Save post" class="save-action" onclick="videoV11ToggleSave(this)" type="button"><svg aria-hidden="true" class="save-bookmark" viewBox="0 0 24 24"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.8L6 21Z"></path></svg><span>Save</span></button>
</div>
<div class="latest-comment empty-comment"><span class="comment-username"></span><span class="latest-comment-copy">No comments yet</span><span class="latest-comment-time"></span></div>
<div class="post-footer">
<div class="single-comment-composer">
<div class="comment-composer" onclick="videoV11OpenCommentComposer(this)">
<div class="comment-compact-state"><span class="comment-mini-avatar"></span><span>Say hi... share what you feel</span></div>
<div class="comment-expanded-state"><textarea class="comment-text-area" maxlength="500" onclick="event.stopPropagation()" placeholder="Say hi... share what you feel"></textarea><div class="comment-post-actions"><button class="comment-post-button" onclick="videoV11PostComment(this,event)" type="button">POST</button><button class="comment-close-button" onclick="videoV11CloseCommentComposer(this,event)" type="button">CLOSE</button></div></div>
</div>
<button class="hide-comments-inline" onclick="videoV11HideComments(this)" type="button">Hide comments ↑</button>
<div class="comment-helper"><span>Press Esc to close · ↵ to post</span></div>
</div>
<button class="more-comments" onclick="videoV11ToggleComments(this)" type="button">View 0 more</button>
<div class="post-footer-actions">${attachedLink?`<a class="feed-link-pill" href="${attachedLink}" target="_blank" rel="noopener noreferrer" title="${escapeHTML(attachedLink)}"><span class="feed-link-icon">🔗</span><span class="feed-link-text">${escapeHTML(displayAttachedLink(attachedLink))}</span></a>`:""}<button class="hashtag-link" onclick="togglePostHashtags(this)" type="button"><span class="hashtag-icon">#</span><span>Hashtags</span></button></div>
</div>
<div class="post-hashtags">${selectedHashtags.length?selectedHashtags.map(h=>`<span class="post-hashtag">${h}</span>`).join(""):`<span class="post-hashtag">No hashtags used</span>`}</div>
<section class="comments-section"></section>
<div class="card-bottom-spacer" aria-hidden="true"></div>`;
        ...
        np.dataset.postType="byte";
        np.innerHTML=`<div class="byte-stage">${videoHTML}</div>
...
<div class="post-header-right">
<button aria-label="More options" class="post-menu-button" onclick="byteV11TogglePostMenu(event,this)" type="button">•••</button>
<div class="byte-v11-menu">
<button class="post-menu-action" onclick="editPost(this)" type="button">Edit</button>
<span class="post-menu-divider">|</span>
<button class="post-menu-action post-menu-delete" onclick="deletePost(this)" type="button">Delete</button>
</div>
...
<p class="byte-caption caption">${escapeHTML(text)}</p>
<div class="interactions">
<button class="action-pill byte-like-button" onclick="byteV11ToggleLike(this)" type="button"><svg aria-hidden="true" class="like-heart" viewBox="0 0 24 24"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8Z"></path></svg><span class="like-count">0</span></button>
<button class="action-pill pocket-action" type="button"><svg aria-hidden="true" class="action-icon" viewBox="0 0 24 24"><path d="M4 7h6l2 2h8v10H4Z"></path><path d="M8 4v6"></path></svg><span>Pocket</span></button>
<button class="action-pill community-action" type="button"><svg aria-hidden="true" class="action-icon" viewBox="0 0 24 24"><circle cx="8" cy="8" r="3"></circle><circle cx="17" cy="9" r="2.5"></circle><path d="M3 19c.4-4 2.4-6 5-6s4.6 2 5 6"></path><path d="M13 18c.5-2.8 1.9-4.3 4-4.3 2 0 3.4 1.4 4 4.3"></path></svg><span>Community</span></button>
<button class="reblog-counter" onclick="byteV11IncrementReblog(this)" type="button">↻ <span>0</span> · Reblog</button>
<button class="share-action" type="button">↗ Share</button>
<button aria-label="Save post" class="save-action" onclick="byteV11ToggleSave(this)" type="button"><svg aria-hidden="true" class="save-bookmark" viewBox="0 0 24 24"><path d="M6 4.75A1.75 1.75 0 0 1 7.75 3h8.5A1.75 1.75 0 0 1 18 4.75V21l-6-3.8L6 21Z"></path></svg><span>Save</span></button>
</div>
<div class="latest-comment empty-comment"><span class="comment-username"></span><span class="latest-comment-copy">No comments yet</span><span class="latest-comment-time"></span></div>
<div class="post-footer">
<div class="single-comment-composer">
<div class="comment-composer" onclick="byteV11OpenCommentComposer(this)">
<div class="comment-compact-state"><span class="comment-mini-avatar"></span><span>Say hi... share what you feel</span></div>
<div class="comment-expanded-state"><textarea class="comment-text-area" maxlength="500" onclick="event.stopPropagation()" placeholder="Say hi... share what you feel"></textarea><div class="comment-post-actions"><button class="comment-post-button" onclick="byteV11PostComment(this,event)" type="button">POST</button><button class="comment-close-button" onclick="byteV11CloseCommentComposer(this,event)" type="button">CLOSE</button></div></div>
</div>
<button class="hide-comments-inline" onclick="byteV11HideComments(this)" type="button">Hide comments ↑</button>
<div class="comment-helper"><span>Press Esc to close · ↵ to post</span></div>
</div>
<button class="more-comments" onclick="byteV11ToggleComments(this)" type="button">View 0 more</button>
<div class="post-footer-actions">${attachedLink?`<a class="feed-link-pill" href="${attachedLink}" target="_blank" rel="noopener noreferrer" title="${escapeHTML(attachedLink)}"><span class="feed-link-icon">🔗</span><span class="feed-link-text">${escapeHTML(displayAttachedLink(attachedLink))}</span></a>`:""}<button class="hashtag-link" onclick="togglePostHashtags(this)" type="button"><span class="hashtag-icon">#</span><span>Hashtags</span></button></div>
</div>
<div class="post-hashtags">${selectedHashtags.length?selectedHashtags.map(h=>`<span class="post-hashtag">${h}</span>`).join(""):`<span class="post-hashtag">No hashtags used</span>`}</div>
<section class="comments-section"></section>
...
function editPost(button){
    const post=button.closest(".post"),mode=post.classList.contains("blog-post")?"blog":(post.classList.contains("video-post-v11")||post.classList.contains("video-post"))?"video":(post.classList.contains("byte-post-v11")||post.classList.contains("byte-post"))?"byte":"post",tab=document.querySelector(`.creator-type-tab[data-type="${mode}"]`),composer=document.getElementById("composer"),feed=document.getElementById("feed"),sidebarButton=document.getElementById("sidebarCreateButton");
    ...
    if(mode==="blog"){document.getElementById("blogTitle").value=post.querySelector(".blog-feed-title")?.textContent.trim()||"";const source=post.querySelector(".blog-source-html");document.getElementById("blogBodyEditor").innerHTML=source?source.innerHTML:(post.querySelector(".blog-full-body")?.innerHTML||"");prepareBlogInlineImages();}else{if(mode==="video")document.getElementById("blogTitle").value=(post.querySelector(".video-title")?.textContent||post.querySelector(".video-feed-title")?.textContent||"").replace(/^▶\s*/,"").trim();document.getElementById("mainPostText").value=(post.querySelector(mode==="video"?".video-caption":mode==="byte"?".byte-caption":":scope > .caption")?.textContent||"").trim();}
...
function filterPockets(value=""){const q=value.trim().toLowerCase();document.querySelectorAll("#pocketList .pocket-item").forEach(item=>{const hay=`${item.dataset.pocketName||""} ${item.dataset.pocketCreator||""}`.toLowerCase();item.style.display=!q||hay.includes(q)?"":"none"});document.querySelectorAll("#pocketList .pocket-flyout-section").forEach(section=>{const any=[...section.querySelectorAll(".pocket-item")].some(item=>item.style.display!=="none");section.style.display=any?"":"none"})}
...
document.addEventListener("click",()=>closePocketSettingsMenus());
document.addEventListener("keydown",event=>{if(event.key==="Escape")closePocketSettingsMenus()});
...
desktopSidebar?.addEventListener("wheel",event=>{if(event.target.closest?.(".pocket-flyout")||window.matchMedia("(max-width: 800px)").matches||desktopSidebar.scrollHeight<=desktopSidebar.clientHeight)return;event.preventDefault();const maximum=desktopSidebar.scrollHeight-desktopSidebar.clientHeight;desktopSidebar.scrollTop=Math.max(0,Math.min(maximum,desktopSidebar.scrollTop+event.deltaY))},{passive:false,capture:true});
...
requestAnimationFrame(()=>{syncCreatorVideoControls(document.getElementById('videoPreviewPlayer'));document.querySelectorAll('.video-feed-player video').forEach(syncFeedVideoControls);document.querySelectorAll('.post.byte-post,.post.byte-post-v11').forEach(setupByteCaption);document.querySelectorAll('.post.video-post').forEach(setupVideoDescription);document.querySelectorAll('.post').forEach(updateMoreLink)});
...
/* =========================================================
   LIVE HOME — VIDEO V11 HANDLERS
   Scoped to .video-post-v11. Existing Blog/Regular remain untouched.
========================================================= */
...
function videoV11UpdateCommentCount(post){
  const view=post?.querySelector(".more-comments");
  if(!view)return;
  const count=post.querySelectorAll(".comments-section .comment-item").length;
  view.textContent=`View ${count} more`;
...
function videoV11PostComment(btn,event){
  ...
  section?.appendChild(item);
  field.value="";
  videoV11UpdateCommentCount(post);
  section?.classList.add("active");
  videoV11SyncComments(post,true);
  videoV11CloseCommentComposer(btn,event);
}
...
document.addEventListener("click",event=>{
  const shareButton=event.target.closest(".feed .post.video-post-v11 .share-action");
  if(!shareButton)return;
  const active=shareButton.classList.toggle("shared");
  shareButton.setAttribute("aria-pressed",active?"true":"false");
});
...
document.addEventListener("click",()=>{
  document.querySelectorAll(".video-v11-menu.open").forEach(m=>m.classList.remove("open"));
});

document.addEventListener("keydown",event=>{
  const field=event.target.closest?.(".video-post-v11 .comment-text-area");
  if(!field)return;
  const post=field.closest(".video-post-v11");
  if(event.key==="Escape"){
    event.preventDefault();
    videoV11CloseCommentComposer(field,event);
    return;
  }
  if(event.key==="Enter"&&!event.shiftKey){
    event.preventDefault();
    const button=post?.querySelector(".comment-post-button");
    if(button)videoV11PostComment(button,event);
  }
});
...
/* =========================================================
   LIVE HOME — BYTE V11 HANDLERS
   Scoped to .byte-post-v11. Existing Post/Blog/Video remain untouched.
========================================================= */
...
function byteV11UpdateCommentCount(post){
  const view=post?.querySelector(".more-comments");
  if(!view)return;
  const count=post.querySelectorAll(".comments-section .comment-item").length;
  view.textContent=`View ${count} more`;
...
function byteV11PostComment(btn,event){
  ...
  section?.appendChild(item);
  field.value="";
  byteV11UpdateCommentCount(post);
  section?.classList.add("active");
  byteV11SyncComments(post,true);
  byteV11CloseCommentComposer(btn,event);
}
...
document.addEventListener("click",event=>{
  const shareButton=event.target.closest(".feed .post.byte-post-v11 .share-action");
  if(!shareButton)return;
  const active=shareButton.classList.toggle("shared");
  shareButton.setAttribute("aria-pressed",active?"true":"false");
});
...
document.addEventListener("click",()=>{
  document.querySelectorAll(".byte-v11-menu.open").forEach(m=>m.classList.remove("open"));
});

document.addEventListener("keydown",event=>{
  const field=event.target.closest?.(".byte-post-v11 .comment-text-area");
  if(!field)return;
  const post=field.closest(".byte-post-v11");
  if(event.key==="Escape"){
    event.preventDefault();
    byteV11CloseCommentComposer(field,event);
    return;
  }
  if(event.key==="Enter"&&!event.shiftKey){
    event.preventDefault();
    const button=post?.querySelector(".comment-post-button");
    if(button)byteV11PostComment(button,event);
  }
});
