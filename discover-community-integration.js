...
function unreadCount(slug){
  const read=readIds();
  return allNotifications().filter(item=>item.communitySlug===slug&&!read.has(item.id)).length;
}
...
const hotbar=document.getElementById("discoverPinnedCommunities");
...
function wireDrag(chip){
  ...
  chip.addEventListener("dragend",()=>{
    ...
    const order=[...hotbar.querySelectorAll(".community-chip")].map(item=>item.dataset.slug);
    savePinOrder(order);
  });
}
...
function renderCommunities(){
  ...
  hotbar.innerHTML="";
  const dropdown=document.getElementById("discoverCommunityDropdown");
  ...
  joined.forEach(([slug,item])=>{
    ...
    pin.addEventListener("click",event=>{
      ...
      if(item.pinned&&!order.includes(slug))order.push(slug);
      if(!item.pinned){
      ...
      document.getElementById("discoverCommunityMoreButton")?.setAttribute("aria-expanded","true");
    });
...
function openQuickNotifications(slug,anchor){
  const popup=document.getElementById("discoverCommunityQuickPopup");
  const list=document.getElementById("discoverCommunityQuickList");
  const itemState=state[slug];

  if(!popup||!list||!itemState)return;

  document.getElementById("discoverCommunityQuickTitle").textContent=
    `${itemState.icon||"◉"} ${itemState.name||slug}`;

  list.innerHTML="";

  const items=allNotifications().filter(item=>item.communitySlug===slug).slice(0,4);
  const read=readIds();

  if(!items.length){
    const empty=document.createElement("div");
    empty.className="discover-community-quick-empty";
    ...
    items.forEach(item=>{
      ...
      row.type="button";
      row.className="discover-community-quick-item"+(read.has(item.id)?"":" unread");
      row.innerHTML='<span class="discover-community-quick-item-copy"><strong></strong><span></span></span><small></small>';

      row.querySelector("strong").textContent=item.title;
      row.querySelector(".discover-community-quick-item-copy > span").textContent=item.preview||"";
  ...
  document.getElementById("discoverCommunityQuickFooter").href=
...
document.getElementById("discoverCommunityQuickClose")
  ?.addEventListener("click",closeQuickNotifications);

document.getElementById("discoverCommunityMoreButton")
...
document.addEventListener("click",event=>{
  if(!event.target.closest(".community-menu")){
    document.getElementById("discoverCommunityDropdown")?.classList.remove("active");
    document.getElementById("discoverCommunityMoreButton")?.setAttribute("aria-expanded","false");
  }
  ...
  if(
    popup &&
    !popup.hidden &&
    !event.target.closest("#discoverCommunityQuickPopup") &&
...
function openManageDrawer(){
  const layer=document.getElementById("discoverManageCommunitiesDrawerLayer");
  const frame=document.getElementById("discoverManageCommunitiesDrawerFrame");

  if(!layer||!frame)return;
  if(!frame.src)frame.src="manage-communities.html?panel=1";
...
document.getElementById("discoverOpenManageCommunities")
  ?.addEventListener("click",openManageDrawer);

document.getElementById("discoverManageCommunitiesDrawerClose")
  ?.addEventListener("click",closeManageDrawer);

document.getElementById("discoverManageCommunitiesDrawerBackdrop")
  ?.addEventListener("click",closeManageDrawer);
